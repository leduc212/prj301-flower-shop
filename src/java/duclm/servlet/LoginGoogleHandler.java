...
import duclm.dto.Constants;
import duclm.dto.UserGoogleDTO;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(AccountDAO.getAccountByEmail(user.getEmail())!=null){
                Account acc = AccountDAO.getAccountByEmail(user.getEmail());
                session.setAttribute("loginedUser", acc);  
            } else {
                AccountDAO.insertAccount(user.getEmail(), "", user.getGiven_name()+" "+user.getFamily_name(), "", 1, 0);
                Account acc = AccountDAO.getAccountByEmail(user.getEmail());
                session.setAttribute("loginedUser", acc);  
            }
